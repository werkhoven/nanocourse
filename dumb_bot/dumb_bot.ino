...
long boutLength = 3000;     // maximum length of forward run
...
void loop() {
  ...
  if( (bottomDetector > bottomThresh || !whisker) && dir != 'b' ){  
  ...
  // assign a new maneuver if the maneuver duration is exceeded
   if ( tElapsed > manDur*1000 ){
    ...
    if( prev_dir == 'f' ){
      ...
      manDur = random(0,850);                  // limit spin time to 500-1350ms
      manDur = manDur + 500;
